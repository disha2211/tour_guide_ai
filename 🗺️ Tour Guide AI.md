...
The current implementation is intentionally lightweight, but there are several areas that could be improved for production use.

### Image Recognition

The current approach is **nearest-neighbor image retrieval**, not a trained landmark classifier. Recognition quality therefore depends heavily on the diversity and quality of images stored in the database.

### Retrieval Evaluation

There is currently no formal evaluation framework for:
...
The retrieval layer returns document metadata, but the current UI does not expose detailed citations/source references to the user.

### Scalability

The local ChromaDB architecture is suitable for experimentation and small deployments but would need a more scalable vector infrastructure for a large tourism knowledge base.

### Error Handling

The current application has limited handling for:
